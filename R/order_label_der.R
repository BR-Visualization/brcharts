...
#' head(dot_plot_data)
#'
create_order_label_der <- function(indata, groupvars, groupeff, dataout = NULL, space_btwn_out_yn = "Y") {
  ...
  ordered_data <- indata %>%
    ...
    ungroup() %>%
    arrange(across(all_of(c("type", "factor", "eff_order", "outcome")))) %>%
    distinct(across(all_of(c(groupvars, groupeff)))) %>%
  ...
  processed_data <- indata %>%
    ...
    arrange(neword) %>%
    mutate(allobs = n() / 2) %>%
    group_by(across(all_of(c("factor", "outcome")))) %>%
    mutate(
  ...
  adjust <- processed_data %>%
    distinct(across(all_of(c("type", "factor", "outcome")))) %>%
  ...
  final_data <- processed_data %>%
    ...
    group_by(factor) %>%
    mutate(
      neword = neword - if (space_btwn_out_yn == "Y") adjust_number else 0,
      mins_y = if_else(factor == "Benefit", min(neword), max(neword))
    ) %>%
    mutate(
      neword = if_else(factor == "Benefit",
                       neword - mins_y + 2,
                       neword - mins_y - 2)
    ) %>%
    ungroup()
  ...
  # Log message
  message(glue('[{format(Sys.time(),"%F %T")}] > Dataout object from the create_order_label_der function is created'))
